...
function createPacketBreakTracker(options = {}) {
  ...
  const pruneMiningControllerPending = options.pruneMiningControllerPending
  ...
  function prune(now = Date.now(), options = {}) {
    ...
    const controllerStale =
      typeof pruneMiningControllerPending === 'function'
        ? pruneMiningControllerPending({
            now,
            stalePendingMs: Math.min(
              packetTtl,
              Number(options.miningControllerStalePendingMs) || packetTtl
            )
          })
        : 0
