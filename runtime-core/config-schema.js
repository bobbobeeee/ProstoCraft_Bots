...
function createRuntimeSettings(config, options = {}) {
  ...
  return {
    ...
    MINING_LOOP_IDLE_MS: (() => { const v = Number(timing.miningLoopIdleMs); return Number.isFinite(v) ? Math.max(0, v) : 2 })(),
    ...
    BURST_BREAK_INTERVAL_MS: (() => { const v = Number(timing.burstBreakIntervalMs); return Number.isFinite(v) ? Math.max(0, v) : 1 })(),
    ...
    BURST_LOOK_REFRESH_MS: (() => { const v = Number(timing.burstLookRefreshMs); return Number.isFinite(v) ? Math.max(0, v) : 2000 })(),
    ...
    FAST_DIG_CONFIRM_MS: (() => { const v = Number(timing.fastDigConfirmMs); return Number.isFinite(v) ? Math.max(0, v) : 15 })(),
    FAST_DIG_RETRY_MS: (() => { const v = Number(timing.fastDigRetryMs); return Number.isFinite(v) ? Math.max(0, v) : 5 })(),
