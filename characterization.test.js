...
assertExportNames('./runtime-core/mining-controller', [
...
assert.deepStrictEqual(
  ...
  ),
  {
    ...
    packetSafetySource: null,
    stabilityCooldownReason: null,
    noteNoInternet: false
  }
)

const speedProfile = createSpeedGuardProfile()
rememberSpeedGuardPeak(speedProfile, 750, 1000, 10000, { stickyPeakMemoryMs: 600000 })
rememberSpeedGuardPeak(speedProfile, 580, 120000, 10000, { stickyPeakMemoryMs: 600000 })
assert.strictEqual(getSpeedGuardTargetRate(speedProfile, 0.95), 712.5)
...
assert.strictEqual(classifyHealthEvent({ message: 'speed-guard просадка скорости' }), 'speed-drop')
assert.deepStrictEqual(
  stable(
    getRuntimeRecoveryDecision({
      running: true,
...
recordPacketIncident(governor, 'too many packets', { now: 1000, safeModeMs: 120000 })
assert.deepStrictEqual(
  stable(
    getPacketGovernorLimits(
      governor,
  ...
  ),
  {
    enabled: true,
    mode: 'safe',
    safeRemainingMs: 119000,
    recoveryElapsedMs: 0,
    recoveryProgress: 1,
    lastIncidentAt: 1000,
    lastRecoveryAt: 0,
    lastReason: 'too many packets',
    incidentCount: 1,
    perSecond: 160,
    burst: 42,
    burstWindowMs: 250,
...
const controller = createMiningController({ now: 1000, minSamples: 3, stalePendingWarn: 3 })
controller.budgetScale = 0.8
for (let index = 0; index < 3; index += 1) {
  const key = `target-${index}`
  recordBreakPacketsSent(controller, { positionKey: key, packetCount: 2, now: 1100 + index * 10 })
  recordBreakPacketConfirmed(controller, { positionKey: key, now: 1130 + index * 10 })
}
const controllerDecision = evaluateMiningController(controller, { now: 15000, force: true })
assert.deepStrictEqual(stable(controllerDecision.decisionSnapshot), {
  enabled: true,
  budgetScale: 0.85,
  sustainableRate: 12.857142857142858,
  sentBreakPackets: 6,
  sentBreakAttempts: 3,
  confirmedPacketBreaks: 3,
  confirmationRatio: 1,
  confirmLatencyMs: 30,
  pendingCount: 0,
  fallbackDigCount: 0,
  ...
  window: {
    ...
    fallbackDigCount: 0,
    stalePendingCleared: 0,
    ratePerMinute: 12.857142857142858
  }
})
assert.deepStrictEqual(
  stable(
    getMiningControllerLimits(controller, {
      fastPerSecond: 300,
      fastBurst: 84,
      safePerSecond: 160,
      safeBurst: 42
...
assert.deepStrictEqual(stable(getMiningControllerSnapshot(controller, 16000).window), {
  elapsedMs: 1000,
  sentBreakPackets: 0,
  sentBreakAttempts: 0,
