...

{
  ...
  assert.strictEqual(settings.BREAK_PACKET_MAX_PER_SECOND, 300)
  assert.strictEqual(settings.LIMBO_FALL_TICKS, 128)
  assert.strictEqual(settings.LIMBO_FALL_PACKET_MS, 50)
  assert.strictEqual(settings.SPEED_GUARD_TARGET_RATIO, 0.9)
...

{
  const legacy = normalizeRuntimeConfig(
    {
      timing: {
        ...
        speedGuardAllowedDropPercent: 7
      },
      antibot: {
        limboFallTicks: 96,
  ...
  assert.strictEqual(settings.BREAK_PACKET_MIN_TARGET_COOLDOWN_MS, 8)
  assert.strictEqual(settings.BREAK_PACKET_MAX_PER_SECOND, 300)
  assert.strictEqual(settings.BREAK_PACKET_BURST_LIMIT, 84)
  assert.strictEqual(settings.BREAK_PACKET_SAFE_MAX_PER_SECOND, 160)
  assert.strictEqual(settings.BREAK_PACKET_SAFE_BURST_LIMIT, 42)
  ...
  assert.strictEqual(settings.LIMBO_FALL_TICKS, 128)
  assert.strictEqual(settings.LIMBO_FALL_PACKET_MS, 50)
  assert.strictEqual(settings.SPEED_GUARD_TARGET_RATIO, 0.9299999999999999)
}

{
  const normalized = normalizeRuntimeConfig(
    {
      timing: {
        miningLoopIdleMs: '0',
        speedGuardAllowedDropPercent: 0,
        speedGuardTargetRatio: 0.88
      },
      position: {
        checkInterval: 1000
      },
  ...
  assert.strictEqual(desktop.MINING_LOOP_IDLE_MS, 2)
  assert.strictEqual(desktop.SPEED_GUARD_TARGET_RATIO, 0.99)
