...
const LEGACY_CONFIG_RULES = [
  ...
  { path: 'timing.breakPacketBurstLimit', legacyValues: [18, 28, 42, 64], fallback: 84 },
  {
    ...
    legacyValues: [42, 60, 96, 120, 150, 240],
    fallback: 160
  },
  {
    path: 'timing.breakPacketSafeBurstLimit',
    legacyValues: [10, 15, 24, 32, 40, 68],
    fallback: 42
  },
  ...
  {
    path: 'timing.speedGuardTargetRatio',
    when: value => !Number.isFinite(Number(value)) || Number(value) < 0.9,
    fallback: 0.9
  },
  {
    path: 'timing.speedGuardRateWindowMs',
    when: value => !Number.isFinite(Number(value)) || Number(value) < 30000,
    fallback: 30000
  },
  { path: 'timing.speedGuardStartGraceMs', legacyValue: 45000, fallback: 20000 },
...
const OBSOLETE_CONFIG_PATHS = [
  ...
  'timing.speedGuardMinBlocksPerMin',
  'timing.speedGuardMinLearnRate',
