...
function importantBotLogTail(filePath, lines = 80) {
  ...
  const importantPattern =
    /\] \[(INFO|SUCC|WARN|ERR )\].*(Сервер:|Запуск|Подключен|BotFilter evidence|тип проверки|LimboFilter|Зашёл на подсервер|Кнопка генератора|Запускаю новый движок|Нет доступных|Добыто|RATE\/MIN|ERR|WARN|Не удалось|Позиция подтверждена)/
  return fs
